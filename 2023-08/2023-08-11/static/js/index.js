...
let year=[]; //년도 저장할 배열
let color=[]; //년도 별 색상
let maxmin=[{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0},{max:0,min:0}];

async function getData(){
    var temp = await fetch("../2023-08-11/temperature.json").then((res)=>res.json());
    // console.log(temp);
    return temp;
}
$(async function(){
    ...
    $.each(data,function(i,item){
        var date=item.날짜.split("-"); //각 데이터의 날짜를 - 기준으로 분리배열
        y.add(date[0]); // 년도만 저장
        keyIn(date);
    ...
    // console.log(Number(data[0].날짜.split("-")[1])==1);
...
function draw_data(ctx,month){
    $.each(month,function(i,m){ //월  - 월 부터 시작하는 이유는 그래프의 기준이 월이니까
        $.each(year,function(k,y){ //년
            ...
            ctx.arc(150+t*30,200+100*i, 5,0,2*Math.PI); //원 그리기
            ctx. fillStyle=color[k]; //원 색상정하기
            ctx.fill(); //원 색상 넣기
        });
    });
}
